...
- **The REST API has no "done" filter.** `GET /notifications?all=true` returns dismissed threads alongside in-inbox ones, and the notification JSON itself has no field that says "this is done". The web UI's Inbox view filters client-side using state that isn't surfaced via the API. To avoid redoing the per-thread merge check + DELETE on every run, `gb.state` keeps a tiny local cache (`$XDG_CACHE_HOME/gb/done-threads.json`, default `~/.cache/gb/`) mapping `thread_id → updated_at at dismissal`. `gb.notifications._iter_not_done` consults this cache before every cleanup loop and skips threads whose current `updated_at` is `<=` the cached value; new activity advances `updated_at` past the cache and the thread is re-processed. **Do not** add a "filter for done" in `GitHubClient.get_notifications` — the cache lives at the feature layer because the client is the thin PyGithub wrapper.
